...
$(document).ready(function() {
    ...
    $("#import_skill_list").click(function() {
        var that = this;
        $.ajax({
            type: "post",
            url: exportRoute + '/import_skill_list',
            success: function(data, status) {
                var data = JSON.parse(data);
                if(data.result == 1) {
    ...
    $("#import_skill_level").click(function() {
        var that = this;
        $.ajax({
            type: "post",
            url: exportRoute + '/import_skill_level',
            success: function(data, status) {
                var data = JSON.parse(data);
                if(data.result == 1) {
                    utils.alert(that, "导入成功！");
    ...
    $("#import_skill_effect").click(function() {
        var that = this;
        $.ajax({
            type: "post",
            url: exportRoute + '/import_skill_effect',
            success: function(data, status) {
                var data = JSON.parse(data);
                if(data.result == 1) {
                    utils.alert(that, "导入成功！");
    ...
    $("#export_heroSkills").click(function() {
        var that = this;
        $.ajax({
            type: "post",
            url: exportRoute + '/export_heroSkills',
            success: function(data, status) {
                var data = JSON.parse(data);
                if(data.result == 1) {
                    utils.alert(that, "导出成功！");
